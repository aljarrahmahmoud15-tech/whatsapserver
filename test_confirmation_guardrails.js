...
assert.match(server, /function finalBookingConfirmationText\(\{ orderNo, executorName, downloaderName, consumerName \}\)/);
assert.match(server, /وصلني الآن — تم تثبيت الحجز/);
assert.match(server, /رقم الرحلة:/);
assert.match(server, /الكابتن الأول:/);
assert.match(server, /الكابتن الثاني المنفّذ:/);

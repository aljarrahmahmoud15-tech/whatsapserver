...
assert.ok(source.includes('const captainInvite = new URLSearchParams(location.search).get(\'invite\')'), 'official gateway preserves an individual invite token');
assert.ok(source.includes("?invite='+encodeURIComponent(captainInvite)"), 'official gateway forwards invite token to the selected captain route');
